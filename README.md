...
**Latest release:** [`v2026.9.15-0`](https://github.com/bemindlabs/BWOC-Framework/releases/tag/v2026.9.15-0) (3.2.0) — **BWOC as its own coding agent**: `bwoc` in any repository opens a coding session (auto-detected provider, `bwoc auth`, coding tools, sandboxed chat), plus a lean pass and deprecations toward 4.0, on top of **BWOC 3.0, the compatibility contract**. Every artifact BWOC owns declares its schema, `bwoc migrate` moves an installation forward, and `[plugin].compat` is enforced. 3.x reads everything 2.x wrote; schema 2 goes away in 4.0 — see [`COMPATIBILITY.en.md`](docs/en/COMPATIBILITY.en.md).
