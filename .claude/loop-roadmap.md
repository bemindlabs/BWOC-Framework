...
- [ ] **`bwoc new --languages` flag** — *deferred* for the same reason (no consumer).
...
- [ ] **docs/en/INCARNATION.md** + TH — step-by-step "how to create a new agent" (currently scattered between template script comments, `README.md`, and `CLAUDE.md`).
...
- **~~`docs/README.md` and `docs/README.bad.md` are misnamed~~ — resolved 2026-09-13:** renamed to `modules/agent-template/docs/persona-example.good.md` / `persona-example.bad.md` (kept in `docs/`; no code depended on the path).
...
- **Iter 2** — `bwoc check` implemented. Full parity port of the template neutrality shell script into `crates/bwoc-cli/src/check.rs`. Subcommand wired, tests pass, fmt+clippy clean. Live run against `modules/agent-template` returns 15 PASS / 0 violations.
