...
gate นี้ **เปิดเป็นค่าตั้งต้น** ตั้งแต่ 3.0: `bwoc task add` สร้าง task ที่มี gate
และ `--no-plan` คือการขอยกเว้นอย่างชัดแจ้ง เดิมมันกลับกัน — ต้อง opt-in ด้วย
`--requires-plan` — และการพลิกนี้มีสาเหตุ ไม่ใช่รสนิยม ดู §การถอนคำว่า "เสร็จแล้ว"
ข้างล่างว่าเกิดอะไรขึ้นกับ task ตัวเดียวที่ไม่มี gate ในการรัน loop จริงครั้งแรก

### การถอนคำว่า "เสร็จแล้ว"

gate ข้างบนกันไม่ให้ task ที่ *มี plan gate* complete เองโดยไม่มีคนดู แต่มันไม่กัน task
ที่ **ไม่มี gate** — และ "เสร็จแล้ว" คือคำยืนยันที่ agent ยืนยันผิดได้ การรัน `--loop`
ครั้งแรกกับ task list จริงมาร์คว่า "merge branch X เข้า main" เสร็จแล้ว ทั้งที่ worker
ไม่ได้ merge อะไรเลย และ state machine เดินทางเดียว รายการที่ใช้ร่วมกันจึงถือความเท็จนั้นไว้
พร้อมส่งต่อให้ task ที่มันปลดล็อก

`bwoc task reopen <team> <task> [--reason …]` ถอนคำยืนยันนั้น เป็นคำสั่งของ **ผู้ดูแล**
— ไม่มี `--as` ไม่เช็คสมาชิก — เพราะสมมติฐานคือบันทึกของฟลีตเองผิด มันใช้ lock เดียวกัน
และเส้นทาง load → mutate → save เดียวกับการเขียนอื่นทุกครั้ง การแก้บันทึกจึงไม่กลายเป็น
ข้ออ้างให้ไปแก้ `tasks.jsonl` ด้วยมือ

| reopen ทำอะไร | ทำไม |
|---|---|
| ล้าง `claimed_by` และ `completed_at` | มันอธิบายงานที่ไม่มีใครยืนยันแล้วว่าเกิดขึ้นจริง |
| ล้าง **คำตัดสิน** plan ของ task ที่มี gate | `Some(true)` ที่รอดมาจะทำให้ผู้เคลมคนถัดไป complete ได้โดยไม่ต้องมี plan ใหม่ — ตรงกับ gate ที่ false completion หลบไปพอดี |
| **เก็บ** ข้อความ plan ไว้ | มันคือคำอธิบายของผู้เคลมเอง เป็นหลักฐานให้คนที่มารับงานต่อ ไม่ใช่คำยืนยันที่ต้องถอน |
| **รายงาน** dependent ไม่ cascade | dependent ที่ยังไม่ถูกเคลมจะกลับไปถูกบล็อกเอง (`claim_task` เช็ค deps ใหม่ตอนเคลม) ส่วนตัวที่กำลังทำอยู่เป็นดุลพินิจของผู้ดูแล ไม่ใช่ของคำสั่งนี้ |

บทเรียนจากการรันครั้งนั้นตรงไปตรงมา: **task ที่ไม่มี gate คือ task ที่ model ยืนยันว่าเสร็จได้**
ถ้าตรวจความสำเร็จเองไม่ได้ ให้ใส่ gate
...
| **ส่ง recurring digest** | cron | aggregate → render → deliver | **ส่งแล้ว** เป็น `bwoc digest` — bucket latch หนึ่งต่อ period บน ledger ที่ส่งแล้ว; delivery ลง local sink (stdout / `--out`) เท่านั้น |
...
3. **Phase L3 — Product loops** — **flagship ส่งแล้ว; ที่เหลือ design-gated** flagship **scheduled monitoring/alerting** ส่งเป็น `bwoc monitor` (`crates/bwoc-cli/src/monitor.rs`): probe source บน `Every` ticker แล้ว alert หนึ่งครั้งต่อ transition OK↔TRIP ผ่าน `IdempotencyLedger` แบบ durable (`bwoc-core::idempotency`) รันใน **trusted wrapper** ส่งแค่ scalar (monitor id + exit code) เข้า alert — output ที่ไม่ trust ไม่ถึง model → **ไม่มี trust-model change** product loop ตัวที่สอง คือ **recurring digest** ก็ส่งแล้วเป็น `bwoc digest` (`crates/bwoc-cli/src/digest.rs`): รันคำสั่งของ operator **หนึ่งครั้งต่อ period** (hourly/daily/weekly) ผ่าน bucket-latch ของ `IdempotencyLedger` ตัวเดียวกัน แล้วส่ง output ลง **local sink** (stdout หรือ `--out <file>`) ไม่ใช่ inbox ของ model — จึงไม่ขยับ trust boundary เช่นกัน ส่วน product loop ที่เหลือ — inbound-service กับ A2A-delegation — ยังต้องมี **trust tier กลาง** — principal *act-as-authenticated-user* ที่เสนอไว้ (ยังเป็น Untrusted) + capability grade *act-as-user* แบบ additive (ทั้งคู่เป็นข้อเสนอเชิงออกแบบ ยังไม่ใช่ identifier ในโค้ดวันนี้) — และ **Dispatch seam** ร่วม; เลื่อนไว้จนกว่าจะมี consumer มา drive (track ใน [#452](https://github.com/bemindlabs/BWOC-Framework/issues/452))
