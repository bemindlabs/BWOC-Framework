...
## ช่องทางเข้าจาก connector สาธารณะ (โหมดสาธารณะแบบจำกัด)

connector ที่ตั้ง `[bot] public = true` ยอมให้ **ใครก็ได้** ที่ DM บอต หรือ @mention
บอตในกลุ่ม สั่ง turn ของ agent ได้ ข้อมูลเข้าแบบนี้ไม่ผ่านการยืนยันตัวตนและต้องถือว่า
เป็นศัตรู ดูการตั้งค่าที่ `CONNECTORS`

| ภัยคุกคาม | มาตรการ |
|---|---|
| คนแปลกหน้าชักจูง agent ให้เขียนไฟล์ รันคำสั่ง หรือออกเครือข่าย | session ถูกล็อกเป็นโหมด `plan` ของ harness ก่อนส่งข้อความผู้ใช้ใด ๆ โหมดนี้ใช้ allow-list อ่านอย่างเดียวแบบตายตัว (`read_file`, `list_dir`, `grep`, `memory_read`) ถ้า harness ไม่ยืนยันโหมด session จะไม่ถูกสร้าง (fail closed) |
...
| คนแปลกหน้าได้ context ของสมาชิกในห้องที่แชร์กัน | session สาธารณะใช้ไฟล์แยก (`…-<chat_id>-public.json`) และไม่เข้า team chat ข้อความของคนแปลกหน้าที่ไม่ได้เรียกบอตจะถูกทิ้ง ไม่ถูกบันทึก |
| ข้อความคนแปลกหน้าถูกเข้าใจผิดว่ามีอำนาจของ operator | ทุก turn ที่ถูกบริดจ์เป็น `Principal::Platform` ซึ่งไม่น่าเชื่อถือ |

**ส่วนที่ยังเหลือ** (1) **การใช้ token สิ้นเปลืองโดยเจตนา:** ค่าใช้จ่ายถูกจำกัดแค่ด้วย
เพดานอัตรา × จำนวนผู้ส่งที่ active × ขนาด turn เท่านั้น `--token-budget` ไม่มีผลกับ
`--chat` จึงยังไม่มีเพดาน token ต่อ turn (2) **prompt injection ในคำตอบ:** คนแปลกหน้า
ยังกำหนดสิ่งที่ agent *พูด* ได้ (3) **การอ่านข้อมูล:** เครื่องมืออ่านอย่างเดียวอ่านทุกอย่าง
ในโฟลเดอร์ agent ได้ รวมถึงไฟล์ session ของแชตอื่น อย่าเก็บความลับหรือประวัติส่วนตัว
ไว้ใน agent ที่เปิดโหมดสาธารณะ
