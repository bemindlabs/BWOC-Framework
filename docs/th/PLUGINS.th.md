...
| `llm-backend` | Backend นอกเหนือจากสิบตัวที่ประกาศ (`claude`, `antigravity`, `codex`, `kimi`, `copilot`, `grok`, `ollama`, `openai-compatible`, `openrouter`, `litellm`) | `bwoc spawn` |
| `workflow` | Integration กับระบบภายนอก (issue tracker, code review, CI) | โค้ดของ agent ที่เรียกออก |
| `audit` | การตรวจสอบ workspace ตามมาตรฐานภายนอกจาก **ISO / IEC / IEEE** (ISO 9001, ISO/IEC 20000-1, ISO/IEC 27001, ISO/IEC 29110, ISO/IEC/IEEE 29148, ISO/IEC/IEEE 12207, IEEE 1012) หรือ audit ที่ operator เขียนเอง (license header, doc parity, secret scan) | `bwoc audit` CLI |
...
ประเภท `audit` เพิ่มเข้ามาใน `BWOC-EPIC-2`; เหตุผล (ทำไมเลือก `audit` ไม่ใช่ `compliance` หรือ `policy`) และโรดแมปมาตรฐานที่เป็นแรงจูงใจ ดู [BWOC-19 design note](../../notes/2026-05-26_iso-compliance-plugins.md) kind นี้ครอบทุกรูปแบบการประกาศของสามองค์กรมาตรฐาน software engineering — **ISO** (เช่น 9001), ร่วม **ISO/IEC** (27001, 20000-1, 29110), ร่วม **ISO/IEC/IEEE** (29148 requirements engineering, 12207 software life cycle), และ **IEEE** เดี่ยว (1012 verification & validation) — เพราะ criterion id, ชื่อมาตรฐาน, และ clause เป็นข้อมูลที่ runtime อ่าน ไม่ใช่ข้อจำกัดว่าองค์กรไหน (หรือกี่องค์กร) ประกาศ
...
- **ไม่ใช่ช่องโหว่สำหรับ logic เฉพาะ vendor ใน framework** สิบ backend ที่ประกาศเป็น first-class อยู่ใน spec ไม่ใช่ plugin คำพูดเฉพาะ vendor ใน `AGENTS.md` ยังห้ามอยู่ (**Samānattatā**)
...
emit โดย `gws-sheets get` (อ่าน) `values-get` คืน grid ค่า (`{ range, values: [[…]] }`); verb เขียน (`values-update` / `values-append`) คืน **ใบเสร็จ** — `updated_range`, `updated_rows`, `updated_columns`, `updated_cells` — ไม่ใช่ resource entry

| ฟิลด์ | ชนิด | บังคับ | ความหมาย |
|---|---|---|---|
| `spreadsheet_id` | string | ใช่ | **key เสถียร** — Spreadsheet id |
| `title` | string | ใช่ | ชื่อ spreadsheet (projection ที่เปลี่ยนได้) |
| `sheet_count` | number | ไม่ | จำนวน tab ตัดออกเมื่อไม่ทราบ |
| `web_view_link` | string | ไม่ | URL เปิดในเบราว์เซอร์ ตัดออกเมื่อไม่มี |

### Google Presentation

emit โดย `gws-slides get` (อ่าน) verb เขียน (`batch-update` / `replace-all-text`) คืน **ใบเสร็จ** — `presentation_id`, `requests_applied`, `occurrences_changed` — ไม่ใช่ resource entry

| ฟิลด์ | ชนิด | บังคับ | ความหมาย |
|---|---|---|---|
| `presentation_id` | string | ใช่ | **key เสถียร** — Slides presentation id |
| `title` | string | ใช่ | ชื่อ presentation (projection ที่เปลี่ยนได้) |
| `slide_count` | number | ไม่ | จำนวนสไลด์ ตัดออกเมื่อไม่ทราบ |
| `web_view_link` | string | ไม่ | URL เปิดในเบราว์เซอร์ ตัดออกเมื่อไม่มี |
...
ปลั๊กอินประเภท `memory-backend` ต้องทำงานได้กับ agent ทุกตัวไม่ว่าใช้ backend ใด ปลั๊กอินประเภท `llm-backend` ต้องไม่แอบอ้างเป็นหนึ่งใน backend สิบตัวที่ประกาศ ชื่อ vendor ใน **manifest values** ทนได้เฉพาะใน `description` (ที่อธิบายเป้าหมายของ integration); ที่อื่นยังห้ามอยู่ กฎ **Samānattatā** เดียวกันกับที่ `bwoc check` บังคับใช้กับ `AGENTS.md` อยู่แล้ว
