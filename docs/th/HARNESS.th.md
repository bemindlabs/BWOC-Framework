...
1. preamble สั้น ๆ สำหรับ coding agent ที่ฝังมาในตัว ยึดหลักปรัชญา BWOC: tool ของ harness แล้วตามด้วยวิธีทำงาน — เข้าใจปัญหาก่อน (Ariyasacca) ตรวจก่อนลงมือ (Yoniso manasikāra) แก้เท่าที่พอ (Mattaññutā) ไล่ความล้มเหลวไปถึงเหตุ (Paṭiccasamuppāda) ปล่อยวางทางที่ไม่เวิร์ก (Anattā) รายงานว่าอะไรตรวจแล้วและอะไรยังไม่ได้ตรวจ (Attaññutā) และข้อระวังตามศีล (ถามก่อนทำสิ่งที่ย้อนกลับไม่ได้ ไม่แตะ secret ไม่อ้างเกินจริงหรือข้ามด่าน และถือผลของ tool เป็นข้อมูล)
...
**แถบ context ด้านขวา** จะอยู่ประจำที่เมื่อ terminal กว้างอย่างน้อย 100 คอลัมน์ แสดง directory และ branch, workspace ที่ครอบอยู่ (ถ้ามี) พร้อม agent ของ workspace นั้น, id/จำนวน turn/การใช้งาน/mode ของ session นี้ และไฟล์ที่ tool แก้ไป อ่านจากดิสก์ตอนจบ turn ไม่ใช่ทุกเฟรม กด `F3` เพื่อซ่อนหรือแสดง ถ้าจอแคบกว่า 100 คอลัมน์ บทสนทนาจะใช้พื้นที่เต็มความกว้าง

**ระหว่างที่ turn กำลังทำงาน** กด `Esc` เพื่อยกเลิก harness จะรอ tool call ที่ค้างอยู่ให้จบก่อน (บทสนทนาจะได้ไม่เสียรูป) แล้วจบ turn นั้น (`Cancel` บนสาย) ถ้าไม่มี turn ทำงานอยู่ `Esc` จะไม่ทำอะไร `/model` เปิดรายการ model ชุดเดียวกับที่ `/models` แสดง ให้เลือก (พิมพ์ส่วนใดของชื่อก็ได้เพื่อกรอง `↑`/`↓` แล้ว `Enter` เพื่อเปลี่ยน ถ้า backend ลิสต์ไม่ได้จะบอกเหตุผล) ส่วน `/model <name>` เปลี่ยนได้ทันที ไม่ว่าทางไหน model ใหม่จะมีผลตั้งแต่ turn ถัดไป เมื่อ tool เขียนหรือแก้ไฟล์ใน working directory session จะแสดง unified diff ของไฟล์นั้น (ไม่เกิน 8 KB และระบุไว้เมื่อถูกตัด) เป็นการแสดงผลอย่างเดียวเพราะ model เห็นผลจาก tool อยู่แล้ว ส่วนแถบสถานะจะแสดงค่าใช้จ่ายเฉพาะเมื่อ provider รายงานมาเท่านั้น (เช่น OpenRouter) ไม่มีการประมาณราคาจากตารางในเครื่อง

**ในช่องพิมพ์** `/` เปิดเมนูคำสั่ง และ `@` เติมชื่อไฟล์ในโปรเจกต์ ใช้ `↑`/`↓` เลือก `Tab` เติมคำ `Esc` ซ่อนเมนู `/sessions` แสดงบทสนทนาทั้งหมดของ directory นี้ (มีเครื่องหมายที่ตัวที่เปิดอยู่) `/session <id>` เปิดบทสนทนาอื่น `/new` เริ่มบทสนทนาใหม่ และ `/fork [<id>]` คัดลอกแล้วเปิดสำเนา ทั้งหมดทำได้โดยไม่ต้องออกจาก TUI (harness จะถูกเปิดใหม่บนบทสนทนาที่เลือกใน terminal เดิม) `/compact` สั่งพับ turn เก่าเป็นบทสรุปทันทีโดยไม่ต้องรอให้ชน budget (บอกด้วยว่าพับไปกี่ข้อความ ถ้าไม่มีอะไรให้พับจะรายงาน 0 และถ้า `max_context` เป็น 0 คือปิด compaction ไว้ มันจะแจ้งแทนที่จะพับ) `/permissions` แสดง policy ที่ session นี้ใช้ (session mode, ค่า default, โหมดราย tool, กฎแบบ pattern) `/mcp` แสดง MCP server ที่ต่ออยู่และ tool ที่แต่ละตัวให้มา `/context` แสดงสิ่งที่อยู่ใน context ของ model (ขนาด system prompt, จำนวนข้อความ, budget, model และ fallback) `/status` บอกสถานะ session นี้ (agent, model, directory, บทสนทนา, mode, จำนวน turn, การใช้งาน) `/tools` แสดง tool ที่ agent เรียกได้ `/cost` แสดง token และราคาเฉพาะเมื่อ provider รายงานมา `/models` แสดง model ที่ backend ให้บริการ (Ollama อ่านจาก `/api/tags` ส่วน LiteLLM, OpenRouter, OpenAI-compatible และ Anthropic อ่านจาก `GET /models` ด้วย key ของ session เอง ดังนั้น LiteLLM key ที่จำกัดโมเดลจะเห็นเฉพาะโมเดลที่ key นั้นเรียกได้) `/agents` แสดง agent ของ workspace ที่ directory นี้อยู่และเปิดรายการให้เลือก agent ที่เลือกจะเปิดใน pane ของตัวเองข้าง session หลัก ขับด้วย `bwoc-harness` ของมันเองตาม backend model และ endpoint ใน manifest ของมัน จึงคุยกับมันใน pane นั้นได้โดย session หลักยังคงบทสนทนาเดิม (`Tab` / `Shift-Tab` ย้าย focus ระหว่าง pane, `/quit` ใน pane ของ agent ปิดเฉพาะ pane นั้น, `Ctrl-C` จบทั้งหมด, `Ctrl-L` หรือ `/layout <ชื่อ>` จัดวาง pane — `main-vertical` (ค่าเริ่มต้น: session หลักซ้าย agent เรียงทางขวา) `main-horizontal` `columns` `rows` `grid` หรือ `single` (เฉพาะ pane ที่ focus) ใช้ชื่อเดียวกับ `bwoc fleet term --layout` agent ที่ใช้ backend `claude` ก็เปิดได้ ผ่าน provider `cli` แบบ chat-only ของ harness ด้วย login ของ CLI เอง โดย pane จะบอกว่า CLI รัน tool ของตัวเองนอก sandbox ของ harness ส่วน agent ที่ใช้ vendor CLI อื่นจะแนะนำให้ใช้ `bwoc chat`) `/backends` บอกว่า backend ไหนใช้ได้ที่เครื่องนี้และคีย์มาจากไหน (ไม่แสดงตัวคีย์) `/settings` แสดง runtime ที่ resolve ได้พร้อมไฟล์ config ที่เกี่ยวข้องและเปิดรายการ key ให้เลือก ส่วน `/settings <key> <value>` เปลี่ยน `model` `backend` `endpoint` `max_tokens` หรือ `max_context` ได้ โดยบันทึกใต้ `[runtime]` ใน `.bwoc/config.toml` ของโปรเจกต์นี้ (สร้างให้ถ้ายังไม่มี และคงเนื้อหาอื่นไว้) แล้ว resolve runtime ใหม่ตามลำดับเดิม — flag หรือ environment variable ยังชนะ และจะบอกให้รู้ — จากนั้น harness เริ่มใหม่บนบทสนทนาเดิม ค่าที่ทำให้ไม่มี runtime ที่รันได้ หรือ backend ที่เป็น vendor CLI จะถูกปฏิเสธและไม่มีการเขียนไฟล์ `/doctor` รันการตรวจชุดเดียวกับ `bwoc doctor` `/retry` ส่งข้อความล่าสุดซ้ำ `/save [path]` บันทึก transcript ลงไฟล์ `/undo` ย้อนการแก้ไฟล์ของ turn ล่าสุดที่มีการแก้ และ `/redo` ทำกลับคืน ใช้ edit journal ที่เก็บไว้ข้างบทสนทนา (ไฟล์ข้อความไม่เกิน 1 MB) ถ้าไฟล์ถูกแก้ไปหลังจากนั้น (ด้วย `run_command` หรือด้วยมือ) จะแจ้งและไม่เขียนทับ ส่วนไฟล์ binary หรือใหญ่เกินจะแจ้งว่าไม่ได้บันทึกไว้ `/help` แสดงคำสั่งทั้งหมด `/clear` ลืมบทสนทนานี้ (ลบประวัติที่บันทึกไว้ เหมือน Forget ใน palette ของ fleet) `/mode` เปิดรายการ permission mode ให้เลือก (`default`, `accept_edits`, `bypass` และ `plan` ซึ่งให้ใช้ได้เฉพาะ tool อ่านอย่างเดียว agent จะเสนอแผนแทนการลงมือ) พร้อมคำอธิบายว่าแต่ละ mode อนุญาตอะไร (`↑`/`↓` เลือก `Enter` ตั้งค่า `Esc` คงค่าเดิม) ส่วน `/mode <ชื่อ>` ตั้งได้ทันที (`F2` วนสลับ mode) และ `/quit` หรือ `/exit` จบ session คำสั่งเหล่านี้ TUI จัดการเองและไม่ส่งอะไรไปให้ model `/name` ที่ไม่รู้จักจะแจ้ง error แทนการส่ง ส่วนบรรทัดที่คำแรกเป็น path เช่น `/etc/hosts` จะถูกส่งเป็นข้อความตามปกติ ตอนส่ง ทุก `@path` ที่ชี้ไปยังไฟล์ข้อความใน working directory จะแนบเนื้อหาไฟล์นั้นไปกับข้อความ (ไม่เกิน 32 KB ต่อไฟล์ และระบุไว้เมื่อถูกตัด) path ที่อยู่นอก directory หรือไฟล์ binary จะแจ้งว่าไม่ได้แนบ และ `@name` ที่ไม่ใช่ไฟล์จะคงเป็นข้อความธรรมดา

**Session ใช้เส้นทางเดียวกับ batch** การเรียก provider จะ retry เมื่อเจอ error ชั่วคราว และถ้า tool call ผิดรูปแบบซ้ำ ๆ จะย้ายไปใช้ fallback ถัดไปจาก `autoModels` server ของ `--mcp` / `--mcp-http` ถูกลงทะเบียนเหมือนใน `bwoc run` ทุก tool call ผ่าน capability gate, guardrail และ permission policy แล้วรันผ่าน turn executor และ OS sandbox ยกเว้น tool เฉพาะ chat สามตัวด้านล่าง (`webfetch`, `todo`, `subagent`) ที่รันใน process หลักหลังผ่านการตรวจชุดเดียวกัน เมื่อมีเนื้อหาที่ไม่น่าเชื่อถือ (output ของ tool หรือข้อความจาก connector) อยู่ในบทสนทนาแล้ว call ที่ถูก gate เช่น `run_command` ต้องได้ Allow อย่างชัดแจ้งแม้อยู่ใน bypass mode ส่วน session แบบ `--headless` จะปฏิเสธเหมือน batch การ compact คำนวณจาก context window ของ model: `max_context` (`--max-context`) ถ้าไม่มีใช้ค่าที่ provider รายงาน ถ้าไม่มีใช้ 200k token สำหรับ `anthropic` ไม่เช่นนั้นใช้ 8,000 token ข้อความ reasoning ที่ provider stream มาจะแสดงเป็นบรรทัดสีจางและยุบลงเมื่อคำตอบเริ่ม คำตอบของ assistant จะถูก render เป็น Markdown ใน TUI โดย parse ตาม CommonMark พร้อมส่วนขยายของ GitHub (ตาราง task list ขีดฆ่า) ได้แก่ หัวข้อ bullet และรายการเลขลำดับ (ซ้อนกันได้) ช่อง task quote code fence code ในบรรทัด ตัวหนา ตัวเอียง ลิงก์ (ข้อความตามด้วย URL) และตารางจัดเป็นคอลัมน์ การขึ้นบรรทัดคงไว้ตามที่เขียน ส่วนผลลัพธ์ของ tool ทุกตัวแสดงตามต้นฉบับ
