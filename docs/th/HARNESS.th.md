...
| **Team chat broadcast** | session แบบ `--chat --team-chat <path>` ใช้ `chat.jsonl` แบบ append-only ร่วมกันของทีม: ข้อความจากเพื่อนร่วมทีมที่โพสต์ตั้งแต่ turn ก่อนถูก inject เป็น system note "Team conversation" ก่อนแต่ละ turn แล้ว append คำตอบของ agent ต่อท้ายหลัง turn เป็น opt-in — ไม่มี flag = session เดี่ยว; agent ไม่เห็นข้อความของตัวเอง echo กลับ | สังฆะ + กัลยาณมิตร | P3 |
