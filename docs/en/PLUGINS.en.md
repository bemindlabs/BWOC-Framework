...
| `llm-backend` | Backends beyond the ten declared (`claude`, `antigravity`, `codex`, `kimi`, `copilot`, `grok`, `ollama`, `openai-compatible`, `openrouter`, `litellm`) | `bwoc spawn` |
| `workflow` | External system integrations (issue trackers, code review, CI) | The agent calling out |
| `audit` | Inspection of the workspace against external standards from **ISO / IEC / IEEE** (ISO 9001, ISO/IEC 20000-1, ISO/IEC 27001, ISO/IEC 29110, ISO/IEC/IEEE 29148, ISO/IEC/IEEE 12207, IEEE 1012) or operator-authored audits (license headers, doc parity, secret scans) | `bwoc audit` CLI |
...
The `audit` kind was added in `BWOC-EPIC-2`; for the rationale (why `audit`, not `compliance` or `policy`) and the standards roadmap that motivates it, see the [BWOC-19 design note](../../notes/2026-05-26_iso-compliance-plugins.md). The kind spans every publication shape across the three software-engineering standards bodies — **ISO** (e.g. 9001), joint **ISO/IEC** (27001, 20000-1, 29110), joint **ISO/IEC/IEEE** (29148 requirements engineering, 12207 software life cycle), and standalone **IEEE** (1012 verification & validation) — since a criterion id, standard designation, and clause reference are data the runtime reads, not a constraint on which body (or how many) publishes the standard.
...
- **Not a loophole for vendor-specific framework logic.** The ten declared backends are first-class and live in spec, not as plugins. Vendor phrasing in `AGENTS.md` is still forbidden (**Samānattatā**).
...
Emitted by `gws-sheets get` (read). `values-get` returns a value grid (`{ range, values: [[…]] }`); the write verbs (`values-update` / `values-append`) return a write **receipt** — `updated_range`, `updated_rows`, `updated_columns`, `updated_cells` — not a resource entry.

| Field | Type | Required | Semantics |
|---|---|---|---|
| `spreadsheet_id` | string | yes | **Stable key** — the Spreadsheet id. |
| `title` | string | yes | Spreadsheet title (mutable projection). |
| `sheet_count` | number | no | Number of tabs. Omitted when unknown. |
| `web_view_link` | string | no | Browser URL for the spreadsheet. Omitted when absent. |

### Google Presentation

Emitted by `gws-slides get` (read). The write verbs (`batch-update` / `replace-all-text`) return a write **receipt** — `presentation_id`, `requests_applied`, `occurrences_changed` — not a resource entry.

| Field | Type | Required | Semantics |
|---|---|---|---|
| `presentation_id` | string | yes | **Stable key** — the Slides presentation id. |
| `title` | string | yes | Presentation title (mutable projection). |
| `slide_count` | number | no | Number of slides. Omitted when unknown. |
| `web_view_link` | string | no | Browser URL for the presentation. Omitted when absent. |
...
A `memory-backend` plugin must work for any agent regardless of backend. An `llm-backend` plugin must not pretend it is one of the ten declared backends. Vendor names in plugin **manifest values** are tolerated only inside `description` (where they describe the integration target); they remain forbidden anywhere else. This is the same **Samānattatā** rule that `bwoc check` already enforces on `AGENTS.md`.
