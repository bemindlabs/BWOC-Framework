...
| `audit` | Inspection of the workspace against external standards from **ISO / IEC / IEEE** (ISO 9001, ISO/IEC 20000-1, ISO/IEC 27001, ISO/IEC 29110, ISO/IEC/IEEE 29148) or operator-authored audits (license headers, doc parity, secret scans) | `bwoc audit` CLI |
...
The `audit` kind was added in `BWOC-EPIC-2`; for the rationale (why `audit`, not `compliance` or `policy`) and the standards roadmap that motivates it, see the [BWOC-19 design note](../../notes/2026-05-26_iso-compliance-plugins.md). The kind spans all three software-engineering standards bodies — **ISO** (e.g. 9001), joint **ISO/IEC** (27001, 20000-1, 29110), and joint **ISO/IEC/IEEE** (29148, requirements engineering) — since a criterion id, standard designation, and clause reference are data the runtime reads, not a constraint on which body publishes the standard.
