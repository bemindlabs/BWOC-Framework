...
>
> - The session is locked to **read-only tools** (the harness `plan` mode: `read_file`, `list_dir`, `grep`, `glob`, `memory_read`, plus the in-memory session `todo` list). Writes, `run_command`, git, delegation, `webfetch`, `subagent` and MCP tools are refused. If the harness doesn't confirm the mode, the session is not created.
