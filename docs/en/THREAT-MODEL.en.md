...
| Threat | Mitigation |
|---|---|
| A stranger steers the agent into writing files, running commands, or reaching the network | The session is locked to harness `plan` mode, a fixed read-only allow-list (`read_file`, `list_dir`, `grep`, `glob`, `memory_read`, and the in-memory session `todo`; `webfetch` and `subagent` are excluded), before any user text is sent. If the harness doesn't confirm the mode, the session is not created (fail closed). |
