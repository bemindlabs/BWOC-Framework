...
**A context pane** sits fixed on the right when the terminal is at least 100 columns wide: the directory and its branch, the workspace above it (if any) and that workspace's agents, this session's id, turns, usage and mode, and the files tools have changed. It reads from disk at turn boundaries, not per frame, and `F3` hides or shows it; below 100 columns the conversation keeps the whole width.

**While a turn runs**, `Esc` cancels it: the harness finishes the tool call in flight so the conversation stays well-formed, then ends the turn (`Cancel` on the wire). Between turns `Esc` does nothing. `/model` opens a picker of the models `/models` would list (type to filter by any part of the id, `↑`/`↓`, `Enter` switches; a backend that cannot list says why) and `/model <name>` switches directly; either way the new model is used from the next turn. When a tool writes, edits or multi-edits a file inside the working directory, the session shows a unified diff of that file (capped at 8 KB, marked when cut) — display-only, since the model already has the tool result. The status line shows a session cost only when the provider reports one (OpenRouter does); nothing is estimated from a local price table.

**In the input line**, `/` opens a command menu and `@` completes project files; `↑`/`↓` pick, `Tab` completes, `Esc` hides the menu. `/sessions` lists this directory's conversations (the open one marked), `/session <id>` opens another, `/new` starts one and `/fork [<id>]` copies one and opens the copy — all without leaving the TUI; the harness is reopened on the chosen conversation in the same terminal. `/compact` folds the oldest turns into a summary now instead of waiting for the budget (it reports how many it folded, `0` when there was nothing to fold, and says so rather than folding when `max_context` is `0`), `/permissions` shows the policy this session runs under (session mode, default mode, per-tool modes, pattern rules), `/mcp` lists the MCP servers connected and the tools each contributed, `/context` shows what fills the model's context (system prompt size, messages, budget, model and fallbacks). `/status` reports this session (agent, model, directory, conversation, mode, turns, usage), `/tools` lists the tools the agent can call, `/cost` shows tokens and a price only when the provider reported one, `/models` lists the models the backend serves (Ollama from `/api/tags`; LiteLLM, OpenRouter, OpenAI-compatible and Anthropic from `GET /models` with the session's own key, so a scoped LiteLLM key lists only what it may call), `/agents` lists the agents of the workspace this directory sits in and opens a picker; the one you pick opens in its own pane beside the main session, driven by its own `bwoc-harness` with its manifest's backend, model and endpoint, so you talk to it there while the main session keeps its conversation (`Tab` / `Shift-Tab` move focus between panes, `/quit` in an agent pane closes that pane, `Ctrl-C` ends everything; `Ctrl-L` or `/layout <name>` arranges the panes — `main-vertical` (the default: main left, agents stacked right), `main-horizontal`, `columns`, `rows`, `grid`, or `single` (only the focused pane), the names `bwoc fleet term --layout` uses; an agent on the `claude` backend opens too, through the harness's chat-only `cli` provider on the CLI's own login — the pane says the CLI runs its own tools outside the harness sandbox — while other vendor-CLI agents are pointed to `bwoc chat`), `/backends` shows which backends are usable here and where each key comes from (never the key), `/settings` shows the resolved runtime and the config files behind it and opens a key picker, and `/settings <key> <value>` changes `model`, `backend`, `endpoint`, `max_tokens` or `max_context`: it is saved under `[runtime]` in this project's `.bwoc/config.toml` (created if missing, other contents kept), the runtime is re-resolved with the usual precedence — a flag or environment variable still wins, and it says so — and the harness restarts on the same conversation; a value that leaves no runnable runtime, or a vendor-CLI backend, is refused and nothing is written, `/doctor` runs the same checks as `bwoc doctor`, `/retry` sends the last message again, `/save [path]` writes the transcript. `/undo` takes back the file changes of the last turn that made any and `/redo` reapplies them — an edit journal beside the conversation, text files up to 1 MB; a file changed since (by `run_command`, or by you) is reported and left alone rather than overwritten, and a binary or oversized write is reported as not journalled. `/help` lists the commands, `/clear` forgets this conversation (deleting its saved history, like the fleet palette's Forget), `/mode` opens a picker of the permission modes (`default`, `accept_edits`, `bypass`, and `plan`, where only read-only tools run and the agent proposes rather than acts), each with what it allows (`↑`/`↓`, `Enter` sets, `Esc` keeps the current one), and `/mode <name>` sets one directly (`F2` cycles them), and `/quit` or `/exit` ends the session. The TUI handles these itself and sends nothing to the model; an unknown `/name` is reported rather than sent, and a line whose first word is a path such as `/etc/hosts` goes out as a message. On send, each `@path` naming a text file inside the working directory attaches that file's content to the message (up to 32 KB per file, marked when truncated); a path outside the directory or a binary file is reported and not attached, and an `@name` that isn't a file stays plain text.

**A session runs the batch paths.** Provider calls retry transient errors, and repeated malformed tool calls move to the next `autoModels` fallback. `--mcp` / `--mcp-http` servers register as in `bwoc run`. Every tool call passes the capability gate, guardrails and permission policy, then runs through the turn executor and OS sandbox — except the three chat-only tools below (`webfetch`, `todo`, `subagent`), which run in-process after the same checks. Once untrusted content (a tool output, a connector message) is in the conversation, a gated call such as `run_command` needs an explicit Allow, even in bypass mode; a `--headless` session denies it, as batch does. Compaction is sized from the model's context window: `max_context` (`--max-context`), else what the provider reports, else 200k tokens for `anthropic`, else 8,000 tokens. Reasoning text streamed by the provider shows as a dimmed line that collapses when the answer starts. An assistant turn is rendered as Markdown in the TUI — parsed as CommonMark with GitHub tables, task lists and strikethrough: headings, bullets and numbered lists (nested), task boxes, block quotes, fenced and inline code, bold, italic, links (text then URL) and tables as aligned columns; line breaks are kept as written, and every tool result is shown verbatim.
