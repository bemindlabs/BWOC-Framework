...
- **A tool call left as text names the likely cause.** The warning (#403) blamed a weak model; the usual cause is a server tool-call parser that does not match the model's format (vLLM `--tool-call-parser hermes` on Qwen3, which emits XML), so it now says that first.
