...
- **Plan mode in `--chat`.** A fourth permission mode (`SetMode "plan"`): read-only tools run, but any **mutating** tool (`write_file` / `edit_file` / `run_command` / `git` / `memory_write`) is refused before the permission gate with a note telling the model to present its plan instead of acting. Switch back to `default` / `accept_edits` / `bypass` to execute. Adapted from openclaude's plan mode; reuses the existing `SetMode` / `ModeChanged` wire.
