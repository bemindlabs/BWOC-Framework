...
- **Chat connectors — keyring token resolution (the architect's "keyring default").** `bwoc-connect` now resolves the bot token from the **OS keyring first** (service `bwoc/<platform>`, account = the agent dir's basename), falling back to the platform env var (`TELEGRAM_BOT_TOKEN` / `DISCORD_BOT_TOKEN`) for headless hosts. Per-OS backends so it compiles on all three CI legs (macOS `apple-native`, Windows `windows-native`, Linux `sync-secret-service` + `crypto-rust` — pure-Rust, no system libdbus/openssl). A missing/locked keyring is never fatal: it falls through to the env var, and an absent token reports both sources. Completes the token story (env was the interim PR1 path).
