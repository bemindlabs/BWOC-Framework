#!/usr/bin/env bash
#
# audit-iso-iec-ieee-12207 — runtime. Emits attestation findings per BWOC-27 schema.
#
# Reads operator-declared attestations from .bwoc/workspace.toml under
# [[plugins.audit-iso-iec-ieee-12207.attestations]]. Matches each entry's criterion_id
...
# Design: the shared attestation runtime (notes/2026-05-27_9001-runtime-attestation-source.md); this lane reuses it for ISO/IEC/IEEE 12207.
#
...
if [[ -z "${BWOC_WORKSPACE:-}" ]]; then
  echo "audit-iso-iec-ieee-12207: BWOC_WORKSPACE is unset" >&2
  exit 1
fi
...
if [[ ! -f "$CRITERIA_FILE" ]]; then
  echo "audit-iso-iec-ieee-12207: missing $CRITERIA_FILE" >&2
  exit 1
fi
...
# -- Parse workspace.toml [[plugins.audit-iso-iec-ieee-12207.attestations]] -----------
#
...
ATTESTATIONS_TSV="$(mktemp -t audit-iso-iec-ieee-12207-attestations.XXXXXX)"
...
while IFS=$'\t' read -r id severity; do
  ...
  if [[ -n "$have_attestation" ]]; then
    ...
    remedy="Provide a signed attestation in .bwoc/workspace.toml under [[plugins.audit-iso-iec-ieee-12207.attestations]] with criterion_id=\"${id}\", statement, signer, and signed_at (ISO 8601 date) to satisfy this criterion."
    printf '\n  {"criterion_id":"%s","severity":"%s","status":"fail","evidence":{"kind":"file","value":".bwoc/workspace.toml"},"remedy":"%s"}' \
      "$(json_escape "$id")" \
      "$(json_escape "$severity")" \
      "$(json_escape "$remedy")"
