...
pub const HEADLINE: &str = concat!(
    ...
    " — a chat session you can steer: `Esc` cancels a turn, `/model` switches models, tool edits show a diff, `/session` moves between conversations, and `/undo` takes file changes back"
);
