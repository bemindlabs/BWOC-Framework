...
pub struct ExecutorOutcome {
    ...
    /// macOS reports `WriteConfineOnly` (write-confine + network egress-deny + a
    /// selective secret read-denylist, #329; full read jail is Linux-only — see
    /// `jail` docs).
...
fn roundtrip(
    ...
    // write-confinement sandbox-exec wrapper + network egress-deny + a selective
    // secret read-denylist (#329; full read jail is Linux-only — see `jail`
