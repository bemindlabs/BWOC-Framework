...
- **`tools`** — `read_file`, `write_file`, `edit_file`, `multi_edit`, `list_dir`, `grep`, `glob`, `run_command`, `git`, `run_gates`, `bwoc_task`, `bwoc_send`, `bwoc_run`, `memory_read`, `memory_write`; `--chat` sessions add `webfetch` and the session-scoped `todo` / `subagent`. `registry` builds the schema list and dispatches; `auth` is the OS-keyring `CredentialBroker`.
...
Optional features: `otel` (OTLP export), `test-redteam` (the hostile-child escape suite — `cargo test -p bwoc-harness --features test-redteam --test sandbox_escape`). The default build pulls none of them.
