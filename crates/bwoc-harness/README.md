...
- **`tools`** — `read_file`, `write_file`, `edit_file`, `list_dir`, `grep`, `run_command`, `git`, `run_gates`, `bwoc_task`, `bwoc_send`, `bwoc_run`, `memory_read`, `memory_write`. `registry` builds the schema list and dispatches; `auth` is the OS-keyring `CredentialBroker`.
