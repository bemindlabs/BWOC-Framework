...
**Software-Version:** `2.40.0`   *(canonical in `Cargo.toml` — bumped on `.rs` / `.toml` edits)*
...
**Latest release:** [`v2026.7.25-2`](https://github.com/bemindlabs/BWOC-Framework/releases/tag/v2026.7.25-2) *(2026-07-25 — **2.40.0**; minor — **Resource Protocol live end-to-end** (slices B+C): `bwoc resource advertise` publishes a host's free-compute/memory offer to the `bwoc-gateway` broker (new TTL-evicted `/v1/resource/*` registry) and `bwoc resource discover --kind compute --gpu-vram N` finds matching live offers across the fleet, best-fit first. The discovery loop now works host-to-host; `claim`→lease + `compute` offload execution are slice D. Prior: `v2026.7.25-1` 2.39.0 — Resource Protocol slice A (snapshot + gate-check))*
**Specification:** [`AGENTS.md`](modules/agent-template/AGENTS.md) v2.0
**Last-Updated:** `2026-07-25T07:59:29Z`   *(UTC, ISO 8601 — stamped on every edit)*
