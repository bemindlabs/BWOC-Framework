...
**Latest release:** [`v2026.8.20-1`](https://github.com/bemindlabs/BWOC-Framework/releases/tag/v2026.8.20-1) *(2026-08-20 — **2.44.1** — **packaging fix**: `bwoc-harness` now ships in every release archive and install path, with packaging asserting all three binaries (#460/#461) — before this, any non-checkout install had a `bwoc` that could not spawn the agentic loop; plus the corrected workspace `repository` URL (#463), a README for every crate behind a gate (#462), and a refreshed root README (#464). Prior: `v2026.8.20-0` 2.44.0 — **Loop-Engineering L3 product loops**: `bwoc monitor` (#454) + `bwoc digest` (#456) on the durable `IdempotencyLedger` (#453), plus the inert act-as-user capability tier (#457); `v2026.8.19-0` 2.43.0 — Loop-Engineering L1/L2 + the `bwoc loop` control-center TUI)*
