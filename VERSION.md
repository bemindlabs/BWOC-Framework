...
**Software-Version:** `3.6.0`   *(canonical in `Cargo.toml` — bumped on `.rs` / `.toml` edits)*
...
**Latest release:** [`v2026.9.21-1`](https://github.com/bemindlabs/BWOC-Framework/releases/tag/v2026.9.21-1) *(2026-09-20 — **3.6.0** — a session that can answer for itself: thirteen `/` commands (`/status` `/tools` `/cost` `/models` `/backends` `/settings` `/doctor` `/permissions` `/mcp` `/context` `/compact` `/retry` `/save`) and a fixed context pane (project · workspace · agents · session · changed files). Prior: `v2026.9.21-0` (also cut 2026-09-20) — **3.5.0** — a chat session you can steer: `Esc` cancels a turn, `SetModel` / `/model`, a unified diff per file-mutating tool, provider-reported cost, `/sessions` `/session` `/new` `/fork` in the TUI, Markdown rendering, and `/undo` / `/redo` of a turn's file changes (runtime R4b–R5). Prior: `v2026.9.20-0` 3.4.0 — `/` commands and `@` file mentions; `v2026.9.19-0` 3.3.0 — several conversations per directory; `v2026.9.13-0` 3.0.0 — **BWOC 3.0, the compatibility contract**)*
**Specification:** [`AGENTS.md`](modules/agent-template/AGENTS.md) v3.0
**Last-Updated:** `2026-09-20T13:03:30Z`   *(UTC, ISO 8601 — stamped on every edit)*
