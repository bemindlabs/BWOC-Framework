...
**Software-Version:** `2.39.0`   *(canonical in `Cargo.toml` — bumped on `.rs` / `.toml` edits)*
...
**Latest release:** [`v2026.7.25-1`](https://github.com/bemindlabs/BWOC-Framework/releases/tag/v2026.7.25-1) *(2026-07-25 — **2.39.0**; minor — **Resource Protocol** (`bwoc resource`, slice A): a light host borrows compute/memory from a heavy fleet host through the `bwoc-gateway` broker under a refuse-by-default provider sharing gate — a signed, time-bounded lease over a typed resource (`compute`/`kv`/`knowledge`). Slice A ships `snapshot` (GPU/CPU/RAM probe) + `gate-check` (dry-run your sharing caps); broker + offload land in slices B/C. Prior: `v2026.7.25-0` 2.38.0 — `bwoc accounting` gated financial-write CLI)*
**Specification:** [`AGENTS.md`](modules/agent-template/AGENTS.md) v2.0
**Last-Updated:** `2026-07-25T07:09:16Z`   *(UTC, ISO 8601 — stamped on every edit)*
