...
- **Latest = newest mtime**, not newest id. The harness rewrites the file every turn, so mtime is "last used" with no index to keep in sync.
...
- **3.2 migration is lazy**: the old file moves in the first time its directory is opened (`prepare`). `list` shows it before then, and `list --all` shows unmoved files as "(not opened since 3.2)" since a hash can't be reversed to a path.
