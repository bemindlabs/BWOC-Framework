...
- **Policy lists**: `glob` joins `PURE_READ_TOOLS` (with egress fixtures), plan mode and the chat allow list. `todo` joins plan mode and the allow list; `subagent` joins the allow list only. `multi_edit` is `WorktreeWrite`, is auto-approved in `accept_edits`, and the Adinnādāna secret scan covers every `edits[].new_string`. `webfetch` stays `ask` / `Gated`.
