...
## Root cause
The third assertion writes an executable hook (`#!/bin/sh\nexit 0`, chmod 755) and **immediately** exec's it via `run_task_hook`. On Linux under parallel test load this intermittently returns **ETXTBSY ("Text file busy", errno 26)** — the classic write-then-exec race where the writer's `close()` hasn't fully settled when `execve()` runs. `Command::output()` then errors and `is_ok()` fails.
